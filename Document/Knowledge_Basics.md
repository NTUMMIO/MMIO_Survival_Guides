# Knowledge
> ## Latest Update: 2025-08-27


## MMIO Lab

[[Lab Website](https://sites.google.com/view/mmio-lab)]


## MMIO Lab Members' Projects

### MRI-Styled PET 

*We appreciate the contributions of **林雨農 Yu-Nong Lin** to this project.*

[[GitHub](https://github.com/NTUMMIO/MRI-styled-PET)] A Dual Modality Fusion Approach to Positron Emission Tomography Partial Volume Correction

### Amyloid Tracer Conversion (Currently Unavailable)

*We appreciate the contributions of **蔡承翰 Cheng-Han Tsai** to this project.*

[[GitHub](https://github.com/NTUMMIO/Amyloid_tracer_conversion)] Deep-Learning-Based Amyloid PET Inter-Radiotracer Image Translation on a Paired Dataset

### PAULow 

*We appreciate the contributions of **陳楷霖** to this project.*

[[GitHub](https://github.com/NTUMMIO/PAULow)] Patch-Based Attention U-Net for Low-Resource Learning

### MRS Analysis Toolbox (Currently Unavailable)

*We appreciate the contributions of **王瀚為 Han-Wei Wang** to this project.*

[[GitHub](https://github.com/NTUMMIO/MRS_Analysis_Toolbox)] An Accessible Toolbox for MR Spectroscopy Data Extraction and Analysis


## MMIO Lab Members' Small Tool

### Image Preprocessing 

*We appreciate the contributions of **林欣達 Hsin-Ta Lin** to this small tool.*

[MATLAB] [[SPM Coregister DOF Controller](https://github.com/h-t-lin/SPM_coreg_DOF)] SPM_coreg_DOF

[Python] [[DICOM Manipulation Toolbox](https://github.com/h-t-lin/SPM_coreg_DOF)] DICOM Python Toolbox

### LaTex Thesis templete

*We appreciate the contributions of **林欣達 Hsin-Ta Lin, 李宜師 Yi-Shih Li** to this thesis templete.*

[[GitHub](https://github.com/NTUMMIO/MMIO_MS_Thesis)]


## PET

### Cherry, S.R., Dahlbom, M. (2006). PET: Physics, Instrumentation, and Scanners.
  本書涵蓋各層面的PET相關知識，包含PET影像成像原理和設計儀器的理論等，如果對於PET沒有相關知識背景的可以讀讀看喔!也可以作為課後


## Machine Learning

### 李弘毅 - 機器學習

[[2021](https://speech.ee.ntu.edu.tw/~hylee/ml/2021-spring.php)]
[[2022](https://speech.ee.ntu.edu.tw/~hylee/ml/2022-spring.php)]
[[2023](https://speech.ee.ntu.edu.tw/~hylee/ml/2023-spring.php)]


## Biostatistic

### 台大 - 生物統計學一
[[OCW](https://ocw.aca.ntu.edu.tw/courses/108S101)]